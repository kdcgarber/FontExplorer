...
void setFontName() {
	fontNameList[0] = "\"Default RadioShack Font\"";
	fontNameList[1] = "\"LED-dotmap Font\"";
	fontNameList[2] = "\"Pokemon Font\"";
	fontNameList[3] = "\"Classic 5x7 Font\"";
	...
	fontNameList[14] = "\"MarengiFont\"";
	fontNameList[15] = "\"Revers Default Radio Shack Font\"";
}
...
void clearCurrentChar() {   // this will blank all cells of the current char
	for (int j = 0; j <= 7; j++) {
...
void inverseCurrentChar() {   // this will invert all the cells of the char char
	for (int j = 0; j <= 7; j++) {
		for (int i = 0; i <= 4; i++) {      //loop through the contents of characater letter 
			switch (charlet[i][j][cselect]) {
			case 1:           //light cell
...
void screenprint( int viewData, int charLineCount) {
	...
	wprintf(L"\t Char Set Name: \x1b[1m%-35.35S\x1b[0m\n", fontName.c_str()); // font set name
